...
class output():
    ...
    def psdict(self, data, indent=''):
        # Python 3 handles UTF-8 by default, no need for reload workaround
...
class conn(object):
    ...
    def send(self, data):
        ...
        except (OSError, ConnectionError, UnicodeEncodeError) as e:
            output().errmsg("Failed to send data", str(e))
            return None
        except Exception as e:
            output().errmsg("Unexpected error while sending data", str(e))
            return None
    ...
    def recv(self, bytes):
        try:
            ...
            if self._file:
                data = os.read(self._file, bytes).decode('utf-8', errors='replace')
            # receive data from socket
            else:
                raw_data = self._sock.recv(bytes)
                if not raw_data:
            ...
            return data
        except (OSError, ConnectionError, UnicodeDecodeError) as e:
            output().errmsg("Failed to receive data", str(e))
            return ""
    ...
    def recv_until(self, delimiter, fb=True, crop=True, binary=False):
        ...
        while not r.search(data):
            data += self.recv(buffer_size)  # receive actual data with optimized buffer
            ...
            if wd > self._sock.gettimeout() and wd >= wd_old + limit:
                if len(data) == bytes:
                    ...
                    break
            # visual feedback on large/slow data transfers with progress indicator
            if self.slow(limit, wd) and self.past(0.1, wd) and len(data) > 0:
                # Enhanced progress display with data rate
                elapsed = wd
                rate = len(data) / elapsed if elapsed > 0 else 0
                rate_str = f" ({rate:.1f} B/s)" if rate > 0 else ""
                progress = "⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏"[int(wd * 10) % len("⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏")]
                output().chitchat(f"{progress} {len(data)} bytes received{rate_str}\r", '')
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
